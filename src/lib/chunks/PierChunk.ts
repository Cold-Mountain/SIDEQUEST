...
export class PierChunk extends BaseChunk {
  ...
  /**
   * Search for piers using GeoApify Places API
   */
  public async searchLocations(params: ChunkSearchParams): Promise<ChunkResult> {
    ...
    try {
      ...
      // Categories for pier related places
      const categories = 'man_made.pier';
      ...
      console.log(`Searching for piers near ${params.latitude}, ${params.longitude}`);
      console.log('GeoApify Pier URL:', url.toString());
      ...
      console.log(`GeoApify returned ${data.features?.length || 0} pier locations`);
      ...
      const locations: ChunkLocation[] = data.features.map((feature, index) => {
        ...
        const description = this.generatePierDescription(props);
        
        // Format location string
        const locationStr = this.formatLocationString(props);

        return {
          id: `geoapify-pier-${props.place_id || index}`,
          title: props.name || 'Waterfront Pier',
          description,
          lat: props.lat,
          lng: props.lon,
      ...
      console.log(`Found ${locations.length} pier locations`);
      ...
      console.error('Error searching for piers:', error);
      return {
        success: false,
        locations: [],
  ...
  /**
   * Generate description for pier location
   */
  private generatePierDescription(props: GeoApifyPlace['properties']): string {
    const descriptions = [];
    
    // Base description
    descriptions.push('A scenic waterfront pier perfect for strolling, fishing, and enjoying panoramic water views.');
  ...
  private formatLocationString(props: GeoApifyPlace['properties']): string {
    ...
    return parts.length > 0 ? parts.join(', ') : `Pier (${props.lat.toFixed(4)}, ${props.lon.toFixed(4)})`;
  }
}