...
export class GoogleMarinaChunk extends BaseChunk {
  ...
  public async searchLocations(params: ChunkSearchParams): Promise<ChunkResult> {
    ...
    try {
      ...
      url.searchParams.set('keyword', 'marina');
      url.searchParams.set('key', this.apiKey);

      const response = await fetch(url.toString());
      const data: GooglePlacesResponse = await response.json();
      
      console.log(`Google Places returned ${data.results?.length || 0} marina locations`);
      ...
      const locations: ChunkLocation[] = data.results.slice(0, limit).map((result, index) => ({
        id: `google-marina-${result.place_id || index}`,
        title: result.name || 'Marina',
        description: 'A waterfront marina with boats, docks, and scenic harbor views perfect for maritime adventures.',
        lat: result.geometry.location.lat,
        lng: result.geometry.location.lng,
        location: result.vicinity || `Marina (${result.geometry.location.lat.toFixed(4)}, ${result.geometry.location.lng.toFixed(4)})`,
        type: 'google_marina',
